...
class ChatReadRetrieveReadApproach:
    ...
    """
    system_message_chat_conversation = """Assistant helps to Jindal Steel and Power Limited (JSPL) related questions, and questions are about the anything related to the Jindal Steel and Power Limited (JSPL). Be brief in your answers.
...
Only generate questions and do not generate any text before or after the questions, such as 'Next Questions'"""
    ...
    query_prompt_template = """Below is a history of the conversation so far, and a new question asked by the user that needs to be answered by searching in a knowledge base about wetherford information.
...
"""
    query_prompt_few_shots = [
        {'role' : USER, 'content' : 'What is cardio?' },
        {'role' : ASSISTANT, 'content' : 'Show definition' },
    ]
    ...
    async def run_with_streaming(
        self, history: list[dict[str, str]], overrides: dict[str, Any], username:str = "", origin:str = ""
        ...
        async for event in await chat_coroutine:
            if "content" in event["choices"][0]["delta"]:
                str = str + event["choices"][0]["delta"]["content"]
            if event["choices"][0]["finish_reason"] == "stop":
                que = history[len(history) - 1]["user"]
                # logObj = Log()
                # id = logObj.insert_data(que,str,username,origin)
                # event["choices"][0]["inserted_id"] = id
            yield event
